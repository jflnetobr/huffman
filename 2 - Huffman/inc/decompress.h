...
int is_bit_i_set(unsigned char c, int i); // checks if a given bit in a byte is set

/** @param filePath: the path of the file that is desired to know the size */
/** @return returns file size in bytes */
long long int get_filesize(char filePath[]); // get the number of characters in the input file
...
huff_node *mount_tree(FILE *fileIn, short int *treesize); // construct a huffman tree from given preorder string
...
void fill_file(FILE *fileIn, FILE *fileOut, huff_node *tree, short int trashsize, long long int file_size); // does the entire descompress process and fills the output file with the descompressed file
...
void decompress(char fileInPath[], char fileOutPath[]); // makes all necessary calls to decompress the file
